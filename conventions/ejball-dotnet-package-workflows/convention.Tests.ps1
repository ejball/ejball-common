...
# Define the Pester suite for the ejball package workflow convention.
Describe 'ejball-dotnet-package-workflows convention' {
	BeforeAll {
		...
		function script:InvokeEjballPackageWorkflowConvention {
			param(
	...
	It 'copies the published workflow templates and is idempotent' {
		...
		try {
			...
			InvokeEjballPackageWorkflowConvention -TestDirectory $testDirectory
